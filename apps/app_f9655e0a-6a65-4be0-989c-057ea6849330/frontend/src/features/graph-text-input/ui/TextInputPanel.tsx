...
export const TextInputPanel = React.memo<TextInputPanelProps>(
  ({ className }) => {
    const currentGraph = useGraphStore(state => state.currentGraph);
    const pendingAdjacencyList = useGraphStore(
      state => state.pendingAdjacencyList
    );
    const textInputDirty = useGraphStore(state => state.textInputDirty);
    const setPendingAdjacencyList = useGraphStore(
      state => state.setPendingAdjacencyList
    );
    ...
    }, [currentGraph, resetTextInput, textInputDirty]); // Sync when graph structure changes
    ...
    const handleApply = React.useCallback(async () => {
      ...
      const validationResult =
        validateAdjacencyListFormat(pendingAdjacencyList);
      if (!validationResult.isValid) {
        toast.error(
          `Invalid graph: ${validationResult.errors[0] || 'Unknown validation error'}`
        );
        return;
      }
