...
const getErrorMessage = (error: GraphValidationError) => {
  ...
  switch (error.type) {
    case 'CYCLE_DETECTED': {
      const cyclePath = Array.isArray(details.cyclePath)
        ? details.cyclePath
        : [];
...
export const GraphValidationPanel = React.memo<GraphValidationPanelProps>(
  ({ graph }) => {
        ...
        <div className="space-y-3">
          ...
          {hasErrors && hasServerErrors && hasClientErrors && (
            <div className="text-xs text-gray-500 mt-2">
              <p>
                Some errors detected by server validation, others by client
                validation
              </p>
            </div>
          )}
