...
export const useGraphStore = create<GraphStore>((set, get) => ({
  ...
  setGraph: graph =>
    set(state => {
      ...
      if (!state.textInputDirty && graph) {
        const newAdjacencyList = generateAdjacencyList(
          graph.nodes,
          graph.edges
        );
        return {
  ...
  setPendingAdjacencyList: text =>
    set(state => ({
      ...
      textInputDirty: state.currentGraph
        ? text !==
          generateAdjacencyList(
            state.currentGraph.nodes,
            state.currentGraph.edges
          )
        : text !== '',
    })),
  ...
  syncTextInputFromGraph: () =>
    set(state => {
      if (!state.currentGraph) {
        return { pendingAdjacencyList: '', textInputDirty: false };
      }
      const generatedText = generateAdjacencyList(
        state.currentGraph.nodes,
        state.currentGraph.edges
      );
      return {
